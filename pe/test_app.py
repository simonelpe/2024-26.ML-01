...
def test_hello(client):
    ...
    assert data['result']['latency'] is not None, "La latenza non deve essere None"
    assert isinstance(data['result']['latency'], float), "La latenza è un float"
    assert data['result']['latency'] > 0, "La latenza deve essere positiva"
    assert data['result']['latency'] < 10, "La latenza deve essere inferiore a 10 secondi"
